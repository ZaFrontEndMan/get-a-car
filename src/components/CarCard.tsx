...
const CarCard = ({ car }: CarCardProps) => {
  ...
  const imageSource = car.image || (car.images && car.images[0]) || '/placeholder.svg';
  ...
  return (
    ...
    >
      <div className="relative">
        <img src={imageSource} alt={car.name} className="w-full h-48 object-cover" />
      ...
      <div className="p-4">
        ...
        <div className="flex items-center space-x-4 rtl:space-x-reverse text-gray-600 text-sm mb-3">
          ...
          <div className="flex items-center space-x-1 rtl:space-x-reverse">
            ...
            <span>{transmissionType ? t(transmissionType.toLowerCase()) : t('automatic')}</span>
          </div>
        </div>
        ...
        <div className="space-y-2 mb-4">
          <div className="flex justify-between items-center">
            ...
            <span className="text-lg font-bold text-primary">{t('currency')} {dailyPrice}</span>
          </div>
          {weeklyPrice && (
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600">{t('weekly')}</span>
              <span className="text-sm font-semibold text-gray-800">{t('currency')} {weeklyPrice}</span>
            </div>
          )}
          {monthlyPrice && (
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600">{t('monthly')}</span>
              <span className="text-sm font-semibold text-gray-800">{t('currency')} {monthlyPrice}</span>
            </div>
          )}
        </div>
