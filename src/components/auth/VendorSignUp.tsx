...
const VendorSignUp: React.FC<VendorSignUpProps> = ({ onSuccess }) => {
  ...
  const validateForm = () => {
    ...
    if (!fileData.licenseIdFront || !fileData.licenseIdBack) {
      errs.vendorDocuments = t("businessLicenseRequired");
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formDataObj.append("userType", "vendor");

      const success = await register("vendor", formDataObj);

      if (success) {
        toast.success(t("registrationSuccessful"));
        // Call onSuccess callback if provided (from dialog)
        if (onSuccess) {
          onSuccess();
          setTimeout(() => {
            navigate("/signin");
