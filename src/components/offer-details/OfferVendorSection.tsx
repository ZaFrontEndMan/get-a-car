...
const OfferVendorSection = ({ vendor }: OfferVendorSectionProps) => {
  ...
  const handleViewAllCars = () => {
    // Navigate to cars page with vendor filter
        ...
        <CardContent>
          <div className="flex items-start gap-4">
            <div className="w-16 h-16 rounded-full overflow-hidden bg-gray-200 flex-shrink-0">
              <img 
                src={vendor.image || 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=100&h=100&fit=crop&crop=center'} 
                alt={vendor.name}
              ...
              <div className="flex items-center gap-4 text-sm text-gray-600 mb-3">
                {vendor.rating && (
                  <div className="flex items-center gap-1">
                    ...
                    {vendor.totalReviews && (
                      <span>({vendor.totalReviews} {language === 'ar' ? 'تقييم' : 'reviews'})</span>
                    )}
                  </div>
                )}
                ...
                {vendor.carsCount && (
                  <div className="flex items-center gap-1">
                    ...
                    <span>{vendor.carsCount} {language === 'ar' ? 'سيارة' : 'cars'}</span>
                  </div>
                )}
              </div>
