...
const BookingFlow = ({
  ...
  const { data: carAdditionalServices = [], isLoading: servicesLoading } = useCarAdditionalServices(car.id);
  ...
  useEffect(() => {
    const newDropoffDate = getDefaultDropoffDate(bookingData.pickupDate, rentalDays);
    setBookingData(prev => ({
      ...prev,
  ...
  }, [selectedServices, selectedPickup, selectedDropoff, totalPrice, carAdditionalServices]);

  // Use pickup and dropoff locations from the car data, with fallback
  const pickupLocations = car.pickup_locations && car.pickup_locations.length > 0 
    ? car.pickup_locations 
    : ['Riyadh City Center', 'King Khalid International Airport', 'Al Olaya District', 'King Abdulaziz Road', 'Diplomatic Quarter', 'Al Malaz District'];
  
  const dropoffLocations = car.dropoff_locations && car.dropoff_locations.length > 0 
  ...
  const getServiceDetails = () => {
    ...
    const filteredServices = allServices.filter(service => selectedServices.includes(service.id));
  ...
  const calculateTotalPrice = () => {
    ...
    const servicesPrice = getServiceDetails().reduce((total, service) => total + service.price, 0);
  ...
  const adjustDays = (increment: boolean) => {
    ...
    const newReturnDate = getDefaultDropoffDate(bookingData.pickupDate, newDays);
    setBookingData(prev => ({
      ...prev,
  ...
  const handleSubmitBooking = async () => {
    ...
    try {
      ...
      const servicesTotal = getServiceDetails().reduce((total, service) => total + service.price, 0);
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          customer_name: user.user_metadata?.full_name || user.email || 'Hafez Rahim',
      ...
      toast.success(t('bookingSuccessful') || 'Booking submitted successfully!');
      onClose();
      navigate('/dashboard');
    } catch (error) {
      console.error('Booking error:', error);
      toast.error(t('bookingFailed') || 'Failed to submit booking. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex justify-between items-center">
            <div className="text-center flex-1">
              <h2 className="text-2xl font-bold text-primary">{t('completeBooking')}</h2>
          ...
          {/* Car Info Card */}
          <BookingCarCard car={car} rentalDays={rentalDays} onAdjustDays={adjustDays} />
            ...
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                <div>
                  ...
                  <p className="text-sm text-gray-600">{t('professionalDriversAvailable')}</p>
                </div>
                <Switch checked={needsDriver} onCheckedChange={setNeedsDriver} />
              </div>

              {needsDriver && (
                <div className="mt-4 space-y-4 p-4 bg-white rounded-lg border">
                  <h4 className="font-medium text-sm">{t('driverInformation')}</h4>
                  <div className="grid grid-cols-1 gap-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-2">{t('drivingLicense')} *</label>
                      <Input
          ...
          <div className="bg-white rounded-lg p-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-2">{t('cardType')}</label>
                <Select value={bookingData.cardType} onValueChange={(value) => handleInputChange('cardType', value)}>
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4">
        <div className="bg-gray-50 rounded-lg max-w-2xl w-full max-h-[95vh] overflow-y-auto">
          <div className="p-3 sm:p-6 space-y-4 sm:space-y-6">
            ...
            <div className="flex justify-between items-center">
              <div className="text-center flex-1">
                <h2 className="text-xl sm:text-2xl font-bold text-primary">{t('completeBooking')}</h2>
                <p className="text-gray-600 text-xs sm:text-sm">{t('secureVehicleRental')}</p>
              </div>
              <button onClick={onClose} className="text-gray-500 hover:text-gray-700">
            ...
            <div className="bg-white rounded-lg p-4 sm:p-6">
              <div className="flex items-center gap-2 rtl:gap-reverse mb-4">
                ...
                <h3 className="text-base sm:text-lg font-semibold">{t('driverOption')}</h3>
              </div>
              
              <div className="bg-blue-50 rounded-lg p-4">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <p className="font-medium text-sm sm:text-base">{t('needDriverQuestion')}</p>
                    <p className="text-xs sm:text-sm text-gray-600">{t('professionalDriversAvailable')}</p>
                  </div>
                  <Switch checked={needsDriver} onCheckedChange={setNeedsDriver} />
                </div>
            ...
            <div className="bg-white rounded-lg p-4 sm:p-6">
              <div className="flex items-center gap-2 rtl:gap-reverse mb-4">
                ...
                <h3 className="text-base sm:text-lg font-semibold">{t('paymentInformation')}</h3>
              </div>
