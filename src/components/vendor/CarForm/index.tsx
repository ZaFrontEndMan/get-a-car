...
const CarForm = ({ car, onClose, onSuccess, t }: CarFormProps) => {
  ...
  const buildApiFormData = () => {
    ...
    // Locations
    const pickUp =
      (pickupLocations || []).find((l) => l && l.trim().length > 0) || "";
    const dropOff =
      (dropoffLocations || []).find((l) => l && l.trim().length > 0) || pickUp;
    append("pickUpLocation", pickUp);
    append("dropOffLocation", dropOff);

    // Feature flags
    const hasFeature = (name: string) =>
      (formData.features || []).some((f: string) =>
    ...
    append(
      "airBagCount",
      hasFeature("air bag") || hasFeature("airbag") ? 2 : 0
    );
    ...
    const protectionPrice = (paidFeatures || []).reduce(
      (sum, pf) => sum + (pf.price || 0),
      0
    );
    ...
    // Images
    const imgs: string[] = Array.isArray(formData.images)
      ? formData.images
      : [];
      ...
      <Card className="w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>{car ? t("edit_car") : t("add_car")}</CardTitle>
          ...
          <form onSubmit={handleFormSubmit}>
            ...
            <div className="flex gap-2 pt-4">
              ...
              <Button type="submit" className="flex-1" disabled={isSubmitting}>
                {isSubmitting ? t("saving") : car ? t("update") : t("create")}
              </Button>
            </div>
          </form>
        </CardContent>
