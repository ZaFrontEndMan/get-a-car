...
const BookingStatusFilter = ({ statusFilter, onFilterChange, statusCounts, totalBookings }: BookingStatusFilterProps) => {
  const { t } = useLanguage();

  const filterButtons = [
    { key: 'all', label: t('all'), icon: Filter },
    ...
    <div className="bg-white rounded-xl shadow-sm border p-4">
      <div className="flex flex-wrap gap-2">
        {filterButtons.map((button) => {
          const count = button.key === 'all' ? totalBookings : (statusCounts[button.key] || 0);
          ...
          return (
            <Button
              ...
              size="sm"
              onClick={() => onFilterChange(button.key as BookingStatus | 'all')}
