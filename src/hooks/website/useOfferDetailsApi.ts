    ...
    const fetchOfferDetails = async () => {
      ...
      try {
        ...
        const { data } = await axiosInstance.get(
          `/Client/Website/GetOfferDetails/${id}`
        );
        ...
        const transformedServices =
          offerData.additionalServices?.map((service: any, index: number) => ({
            id: service.id,
            name: service.name || "",
        ...
        const similarResponse = await axiosInstance.get(
          "/Client/Website/GetSimilarOffers",
          {
        ...
        const formattedSimilarOffers = similarOffersData.map(
          (offer: Offer) => ({
            ...
            car: {
              ...
              image:
                offer.offerImage ||
                "/uploads/10b7fec1-615a-4b01-ae08-d35764ce917a.png",
            ...
            discount: Math.round(
              ((offer.oldPricePerDay - offer.totalPrice) /
                offer.oldPricePerDay) *
                100
            ),
        ...
        const transformedOffer = {
          ...
          discount: `${Math.round(
            ((offerData.oldPricePerDay - offerData.totalPrice) /
              offerData.oldPricePerDay) *
              100
          )}%`,
          ...
          policies: [
            ...
            `${t("offerValidUntil")} ${new Date(
              offerData.endDate
            ).toLocaleDateString()}`,
            `${t("discount")}: ${Math.round(
              ((offerData.oldPricePerDay - offerData.totalPrice) /
                offerData.oldPricePerDay) *
                100
            )}% ${t("off")}`,
