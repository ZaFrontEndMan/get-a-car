...
import { calculateBookingPrice, Service, CarPricing, PricingBreakdown } from '../utils/pricingCalculator';
...
export const useOfferDetailsState = () => {
  ...
  const calculateTotalPrice = (offer: any, additionalServices: any[]): PricingBreakdown => {
    if (!offer) {
      return {
        basePrice: 0,
        servicesPrice: 0,
    ...
    const dynamicPricing = calculateBookingPrice(rentalDays, offer.car.pricing, services);
